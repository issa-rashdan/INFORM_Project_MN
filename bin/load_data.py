...
import data.data_paths as data_paths
...
class Echogram():
    ...
    def data_numpy_transformed(self, data_transform=db, frequencies_of_interest=None):
    ...
    def generate_echogram(self, max_length=2000000):
        """ Split echogram into several echograms with max_length """
        if self.shape[0] * self.shape[1] > max_length: 
            total_pixels = self.shape[0] * self.shape[1]
            num_splits = total_pixels // max_length + 1

            for i in range(num_splits):
                label_chunk = np.array_split(self.label_numpy(), num_splits, axis=1)[i]
                data_db_img_chunk = np.array_split(self.data_numpy_transformed(), num_splits, axis=1)[i]
                seabed_chunk = np.array_split(self.get_seabed(), num_splits, axis=0)[i]
                time_vector_chunk = np.array_split(self.time_vector, num_splits, axis=0)[i]
                mask_chunk = np.array_split(self.select_mask(), num_splits, axis=1)[i]
                yield label_chunk, data_db_img_chunk, seabed_chunk, time_vector_chunk, mask_chunk
        else:
            yield self.label_numpy(), self.data_numpy_transformed(), self.get_seabed(), self.time_vector, self.select_mask()
